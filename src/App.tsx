...
const Home = ({ lang, setLang }) => (
    <div className="relative w-full min-h-screen text-foreground bg-background overflow-x-hidden selection:bg-darkPurple selection:text-white font-poppins">
    ...
    <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden opacity-50 dark:opacity-100">
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-darkPurple/[0.05] rounded-full blur-[120px]" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-darkPurple/[0.05] rounded-full blur-[120px]" />
    </div>
      ...
      <section className="min-h-screen flex flex-col items-center justify-center px-6 py-20 relative overflow-hidden">
        ...
        <div className="z-10 flex flex-col items-center flex-grow justify-center">
          <motion.div
            ...
            className="text-[10px] md:text-xs font-bold uppercase tracking-[0.5em] text-darkPurple mb-8 font-montserrat"
          >
            {lang === 'en' ? 'Future-Forward Digital Studio' : 'भविष्य-उन्मुख डिजिटल स्टूडियो'}
          </motion.div>
          
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-extrabold text-center tracking-tighter leading-[0.9] mix-blend-difference font-montserrat text-foreground">
            {lang === 'en' ? 'BEYOND' : 'परे'}<br /> 
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-foreground via-darkPurple to-foreground">
          ...
          <div className="mt-12 flex flex-col sm:flex-row gap-6">
            <a href="#services" className="px-10 py-4 bg-darkPurple text-white rounded-full font-bold uppercase text-[10px] tracking-[0.2em] shadow-[0_0_30px_rgba(46,16,101,0.6)] hover:bg-darkPurple/90 transition-all duration-300">
              {lang === 'en' ? 'Explore Services' : 'सेवाएं देखें'}
            </a>
            <a href="#about" className="px-10 py-4 border border-foreground/10 rounded-full font-bold uppercase text-[10px] tracking-[0.2em] hover:border-darkPurple hover:bg-darkPurple/10 transition-all duration-300">
