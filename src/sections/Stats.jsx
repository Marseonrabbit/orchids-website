...
const Stats = ({ lang }) => {
  const t = {
    ...
    return (
      <div className="py-24 bg-background border-y border-foreground/5 relative overflow-hidden transition-colors duration-500">
        <div className="absolute inset-0 bg-darkPurple/[0.02] opacity-50" />
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
          {t.map((stat, idx) => (
            ...
            >
              <div className="text-5xl md:text-7xl font-extrabold text-foreground mb-4 tracking-tighter font-montserrat group-hover:text-darkPurple transition-colors duration-500">
                {stat.value}
              </div>
