...
const Stats = ({ lang }) => {
  ...
  return (
    <div className="py-24 bg-background border-y border-border/50 relative overflow-hidden">
      <div className="absolute inset-0 bg-foreground/[0.02] opacity-50 dark:opacity-10" />
      <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
        {t.map((stat, idx) => (
          ...
          >
            ...
            <div className="text-[10px] md:text-xs font-bold uppercase tracking-[0.4em] text-muted-foreground font-montserrat transition-colors">
              {stat.label}
            </div>
          </motion.div>
        ))}
