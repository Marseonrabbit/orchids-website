...
const Stats = ({ lang }) => {
  ...
  return (
    <div className="py-24 bg-dark border-y border-white/5 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
        {t.map((stat, idx) => (
          ...
          >
            <div className="text-5xl md:text-7xl font-extrabold text-white mb-4 tracking-tighter font-montserrat">
              {stat.value}
            </div>
            <div className="text-[10px] md:text-xs font-bold uppercase tracking-[0.4em] text-white/40 font-montserrat group-hover:text-white transition-colors">
              {stat.label}
            </div>
          </motion.div>
        ))}
