    ...
    return (
      <SectionWrapper id="services" className="bg-background py-40 relative overflow-hidden transition-colors duration-500">
        ...
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-30">
          <div className="absolute top-1/4 -left-1/4 w-[600px] h-[600px] bg-darkPurple/[0.01] rounded-full blur-[100px] will-change-transform" />
          ...
          <div className="mb-40 flex flex-col lg:flex-row lg:items-end justify-between gap-16">
            <div className="max-w-5xl">
              <motion.div 
                ...
                className="text-[11px] font-black uppercase tracking-[0.8em] text-muted-foreground/50 mb-12"
              >
                {t.tag}
              </motion.div>
              <motion.h2 
                  ...
                  <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                      <div className="flex items-center justify-between mb-20">
                        <span className="text-[10px] font-black text-muted-foreground/20 tracking-[0.5em]">0{service.id}</span>
                        <motion.div 
                          animate={{ 
                            scale: isHovered ? 1.15 : 1,
                            rotate: isHovered ? 5 : 0,
                            backgroundColor: isHovered ? "var(--darkPurple)" : "transparent",
                            color: isHovered ? "#fff" : "currentColor",
                            borderColor: isHovered ? "var(--darkPurple)" : "rgba(var(--foreground), 0.1)"
                          }}
                          transition={{ duration: 0.4, ease: [0.23, 1, 0.32, 1] }}
                          className="w-12 h-12 border border-foreground/10 rounded-full flex items-center justify-center will-change-[transform,background-color] transform-gpu text-foreground"
                        >
                      ...
                      <motion.p 
                        className={`mb-12 text-[13px] leading-loose max-w-[85%] transition-colors duration-300 ${isHovered ? 'text-foreground/80' : 'text-muted-foreground'}`}
                      >
                        {service.desc}
                      </motion.p>
                      ...
                      <div className="flex gap-4 flex-wrap mb-10">
                        {service.tags.map(tag => (
                          <motion.span 
                            key={tag} 
                            className={`text-[9px] font-black uppercase tracking-[0.3em] transition-colors duration-300 ${isHovered ? 'text-darkPurple' : 'text-muted-foreground/40'}`}
                          >
                            {tag}
                          </motion.span>
                        ))}
                      ...
                      <motion.div 
                        className={`flex items-center gap-6 text-[10px] font-black uppercase tracking-[0.5em] transition-colors duration-300 ${isHovered ? 'text-darkPurple' : 'text-muted-foreground/40'}`}
                      >
                        <motion.span 
                          animate={{ x: isHovered ? 8 : 0 }}
                          transition={{ duration: 0.4, ease: [0.23, 1, 0.32, 1] }}
                        ...
                        <motion.div 
                          className={`h-[1px] flex-grow transition-colors duration-300 ${isHovered ? 'bg-darkPurple/30' : 'bg-foreground/5'}`} 
                        />
                      </motion.div>
                  </div>
                </motion.a>
